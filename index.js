...
// - [ ] 1.- Dado el array **numbers** cuyo valor sea [4, 5, 6, 7, 8, 9, 10], crea una función **elevados** q
// ue sea el resultado de elevar cada número a si mismo. 
...
const result2 = foodList.map(food => {
  if (food === 'Pizza') return 'Como soy de Italia, amo comer Pizza';
  if (food === 'Ramen') return 'Como soy de Japón, amo comer Ramen';
  if (food === 'Paella') return 'Como soy de Valencia, amo comer Paella';
  if (food === 'Entrecot') return 'Aunque no como carne, el Entrecot es sabroso';
});


// - [ ] 3.- Dado el  array **staff**, crear un segundo array que forme frases como en el ejemplo accediendo a+
//  las propiedades del objeto proporcionado:==========
...
const result8 = sentenceElements.reduce((acc, word) => {
  return acc + ' ' + word;
});
